...
// let rep = 1;
// // WHILE LOOP needs the condition to be true to keep running 
// while(rep <= 10) {
...
// create an array bills containing 10 bills
...
for (let i = 0; i < bills.length; i++ ) {
    // Reading from bills Array
    console.log(bills[i]);

    // filling the tips array 
    tips.push(calcTip(bills[i]));

    // filling the totals array
...
// create variable sum starts at 0
let sum = 0;

// loop through totals array and add current value to sum
for (let i = 0; i < bills.length; i++) {
    sum += totals[i];
}

// function takes sum of all values in total array and divides by the length of
// array to calculate the average
const calcAverage = arr => sum / totals.length;

// logging sum variable
console.log(sum);

// calling calcAverage function on sum then logging average
console.log(calcAverage(sum[0]));
